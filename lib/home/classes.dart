...
class _MyHomePageState extends State<classes> {
  ...
  Future getemploi() async {
    var url = Uri.parse(
        "$baseURL/emploi/getprofclasse/62406adba98b5349f488d039/62405f921f9afe1ec40c5ff7");
    http.get((url), headers: {"content-type": "application/json"}).then(
        (http.Response response) {
      //print("hello"+response.body.toString());
      // = json.decode(response.body);
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: AppBar(
          toolbarHeight: 80,
          title: Image.asset('assets/images/logoEsprit.png', fit: BoxFit.contain),
...
class _CategoriesScrollerState extends State<CategoriesScroller> {
  ...
  void initState() {
    classelist = [];
    var url = Uri.parse(
        "$baseURL/emploi/getprof/62406adba98b5349f488d039");
    http.get((url), headers: {"content-type": "application/json"}).then(
        (http.Response response) {
      //print("hello"+response.body.toString());
      // = json.decode(response.body);
