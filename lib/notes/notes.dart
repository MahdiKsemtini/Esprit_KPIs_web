...
class _MyHomePageState extends State<notes> {
  ...
  Future getemploi() async {
    listetudiant = [];
    var url = Uri.parse("http://192.168.1.3:4000/students/classe/" + classe1);
          ...
          onPressed: () {
            grades.forEach((element) {
              var response = http.post(
                  Uri.parse("http://192.168.1.3:4000/note/create/"),
                  body: {
...
class _CategoriesScrollerState extends State<CategoriesScroller> {
  ...
  void initState() {
    classelist = [];
    var url = Uri.parse(
        "http://192.168.1.3:4000/emploi/getprof/62406adba98b5349f488d039");
    http.get((url), headers: {"content-type": "application/json"}).then(
        (http.Response response) {
      //print("hello"+response.body.toString());
      // = json.decode(response.body);
